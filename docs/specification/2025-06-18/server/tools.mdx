...
为了信任、安全性和安全性，**应该**始终有人工介入，能够拒绝工具调用。
...
<Note>
  ...
  [注解](/specification/2025-06-18/server/resources#annotations)，这些注解
...
工具**可以**返回指向[资源](/specification/2025-06-18/server/resources)的链接，以提供额外上下文或数据。在这种情况下，工具将返回一个可以被客户端订阅或获取的 URI：
...
[资源](/specification/2025-06-18/server/resources)**可以**被嵌入以使用合适的[URI 方案](./resources#common-uri-schemes)提供额外上下文或数据。使用嵌入资源的服务器**应该**实现 `resources` 能力：
...
嵌入资源支持与常规资源相同的[资源注解](/specification/2025-06-18/server/resources#annotations)，以帮助客户端理解如何使用它们。
