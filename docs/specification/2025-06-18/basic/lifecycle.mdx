...
- 客户端**不应该**在服务器响应 `initialize` 请求之前发送除[ping](/specification/2025-06-18/basic/utilities/ping)之外的其他请求。
- 服务器**不应该**在接收到 `initialized` 通知之前发送除[ping](/specification/2025-06-18/basic/utilities/ping) 和[日志记录](/specification/2025-06-18/server/utilities/logging)之外的其他请求。
...
| 客户端 | `elicitation`  | 支持服务器[引出](/specification/2025-06-18/client/elicitation)请求         |
...
| 服务器 | `completions`  | 支持参数[自动完成](/specification/2025-06-18/server/utilities/completion)  |
...
对于 stdio [传输](/specification/2025-06-18/basic/transports)，客户端**应该**通过以下方式发起关闭：
...
对于 HTTP [传输](/specification/2025-06-18/basic/transports)，关闭通过关闭相关的 HTTP 连接来指示。

## 超时

实现**应该**为所有发送的请求建立超时，以防止挂起的连接和资源耗尽。当请求在超时期间内没有收到成功或错误响应时，发送者**应该**为该请求发出[取消通知](/specification/2025-06-18/basic/utilities/cancellation) 并停止等待响应。

SDK 和其他中间件**应该**允许这些超时在每个请求的基础上进行配置。

实现**可以**选择在接收到对应于请求的[进度通知](/specification/2025-06-18/basic/utilities/progress)时重置超时时钟，因为这意味着工作实际上正在进行。但是，实现**应该**始终强制执行最大超时，无论进度通知如何，以限制行为不端的客户端或服务器的影响。
